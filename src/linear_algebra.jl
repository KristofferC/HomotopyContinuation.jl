...
function geqp3!(A::AbstractMatrix{ComplexF64},
    ...
    info = Ref{BlasInt}()
    for i = 1:2
        ccall((LA.LAPACK.@blasfunc(zgeqp3_), LA.LAPACK.liblapack), Cvoid,
...
function rcond(LU::LA.LU, anorm, work, rwork)
    ...
    ccall((LA.LAPACK.@blasfunc(zgecon_), LinearAlgebra.LAPACK.liblapack), Cvoid,
          (Ref{UInt8}, Ref{Int64}, Ptr{ComplexF64}, Ref{Int64},
           ...
           Ptr{Int64}),
           'I', n, A, lda, anorm, rcond, work, rwork,
           info)
    rcond[]
end
...
function rcond(qr::LA.QRPivoted, work, rwork)
    ...
    rcond = Ref{Float64}(1)
    info  = Ref{BlasInt}()
    ccall((LA.LAPACK.@blasfunc(ztrcon_), LA.LAPACK.liblapack), Cvoid,
          (Ref{UInt8}, Ref{UInt8}, Ref{UInt8}, Ref{BlasInt},
           Ptr{ComplexF64}, Ref{BlasInt}, Ref{Float64}, Ptr{ComplexF64}, Ptr{Float64}, Ptr{BlasInt}),
          'I', 'U', 'N', n,
          qr.factors, m, rcond, work, rwork, info)
    if m == n
...
function inf_norm_est(lu::LA.LU,
                g::Union{Nothing,Vector{<:Real}},
                d::Union{Nothing,Vector{<:Real}},
                work::Vector{<:Complex}, rwork::Vector{<:Real})
    z = ξ = y = work
    x = rwork
